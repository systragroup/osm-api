# from home
# python tests/test.py
import unittest
import os
import geopandas as gpd
import numpy as np

from osm_importer import elevation
from osm_importer.url_list import URL_LIST

import pathlib as pl
from common import get_path

data_path = get_path()


SKIP = False


@unittest.skipIf(SKIP, 'want to skip')
class TestInit(unittest.TestCase):
	# this run only one time.
	# init and read GTFS.
	@classmethod
	def setUpClass(self):
		self.srtm = 'srtm3'
		self.arc = 3
		self.nodes = gpd.read_file(os.path.join(data_path, 'nodes.geojson'))
		self.url_dict = URL_LIST

	def test_get_file_name(self):
		name = elevation.get_file_name(45.443, -73.6)
		self.assertEqual(name, 'N45W074.hgt')
		name = elevation.get_file_name(48.93, 2.332)
		self.assertEqual(name, 'N48E002.hgt')
		name = elevation.get_file_name(-36.959463, 174.856551)
		self.assertEqual(name, 'S37E174.hgt')

	def assertIsFile(self, path):
		if not pl.Path(path).resolve().is_file():
			raise AssertionError('File does not exist: %s' % str(path))

	def test_fetch_data(self):
		url = self.url_dict[self.srtm]['N45W074.hgt']
		elevation.fetch_data(url, data_path)
		path = os.path.join(data_path, 'N45W074.hgt')
		self.assertIsFile(path)

	def test_read_hgt_file(self):
		path = os.path.join(data_path, 'N45W074.hgt')
		alt, lat, lon = elevation.read_hgt_file(path, self.arc)
		self.assertEqual(lon[0], -74)
		self.assertEqual(lon[-1], -73)
		self.assertEqual(lat[0], 45)
		self.assertEqual(lat[-1], 46)

		self.assertAlmostEqual(alt[0, 0], 186)
		self.assertAlmostEqual(alt[0, -1], 364)
		self.assertAlmostEqual(alt[-1, 0], 94)
		self.assertAlmostEqual(alt[-1, -1], 15)

	def test_fill_missing_elevation(self):
		path = os.path.join(data_path, 'N45W074.hgt')
		alt, _, _ = elevation.read_hgt_file(path, self.arc)
		alt = elevation.fill_missing_elevation(alt)
		missing_list = alt[alt == -32768]
		self.assertTrue(len(missing_list) == 0)

	def test_interpolate_elevation(self):
		path = os.path.join(data_path, 'N45W074.hgt')
		alt, lat, lon = elevation.read_hgt_file(path, self.arc)
		lon_list = [-74, -73.5, -73]
		lat_list = [45, 45.5, 46]
		el = elevation.interpolate_elevation(lon_list, lat_list, alt, lon, lat)
		self.assertEqual(el, [186, 24, 15])

	def test_calc_incline(self):
		inc = elevation.calc_incline(np.array(1), np.array(2), np.array(1))
		self.assertEqual(inc, 45)
		inc = elevation.calc_incline(np.array(10), np.array(9), np.array(1))
		self.assertEqual(inc, -45)
		inc = elevation.calc_incline(np.array(10), np.array(10), np.array(31))
		self.assertEqual(inc, 0)
		inc = elevation.calc_incline(np.array([1, 10]), np.array([2, 10]), np.array([1, 1]))
		np.testing.assert_array_equal(inc, np.array([45, 0]))

	def test_get_elevation_from_srtm(self):
		el_dict = elevation.get_elevation_from_srtm(self.nodes)
		self.assertEqual(len(el_dict), len(self.nodes))
		# all element are int.
		self.assertTrue(any([isinstance(el, int) for el in el_dict.values()]))


if __name__ == '__main__':
	unittest.main()
