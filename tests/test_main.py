...
class TestMainCycleway(unittest.TestCase):
	def test_osm_simplify_1(self):
		...
		links, nodes = main.osm_importer(BBOX, HIGHWAY_LIST + ['cycleway'], CYCLEWAY_LIST, False, wd)
		links, nodes = main.osm_simplify(links, nodes, HIGHWAY_LIST, add_elevation, split_direction)
		expected_res = [
			'highway',
			'speed',
			'lanes',
	...
	def test_osm_simplify_4(self):
		...
		links, nodes = main.osm_importer(BBOX, HIGHWAY_LIST + ['cycleway'], CYCLEWAY_LIST, extended_cycleway, wd)
		links, nodes = main.osm_simplify(links, nodes, HIGHWAY_LIST, add_elevation, split_direction)
		expected_res = [
			'highway',
			'speed',
...
class TestMainHighway(unittest.TestCase):
	def test_osm_simplify_1(self):
		add_elevation = True
		split_direction = False
		links, nodes = main.osm_importer(BBOX, HIGHWAY_LIST, None, False, wd)
		links, nodes = main.osm_simplify(links, nodes, HIGHWAY_LIST, add_elevation, split_direction)
		expected_res = [
			'highway',
			'speed',
