...
def merged_reversed_geometries_dict(geojson_dict):
	features = {hash(tuple(tuple(p) for p in feature['geometry']['coordinates'])): feature for feature in geojson_dict['features']}
...
def main_strongly_connected_component(links: gpd.GeoDataFrame, nodes: gpd.GeoDataFrame = None, split_direction: bool = False) -> gpd.GeoDataFrame:
	"""
...
def rectify_geometry_direction(links, nodes):
	...
	log.info(f'{len(links[sliced])} geometry to inverse')
...
def simplify(links, cutoff=10):
	...
	links['weight'] = 1
	log.info(f'{len(sources)} deg 2 nodes')
	path_to_merge = []
	...
	for col1, col2 in [(a, b) for a in links['oneway'].unique() for b in links['highway'].unique()]:
		...
		for origins in batched(filtered_sources, 1000):
			...
			dist_matrix, predecessors = dijkstra(csgraph=mat, directed=True, indices=origin_sparse, return_predecessors=True, limit=cutoff)
		...
		if len(unfounds_origins) > 0:
			log.info(f'find path with large cutoff for {len(unfounds_origins)} origins')
			origin_sparse = [node_index[x] for x in unfounds_origins]
			dist_matrix, predecessors = dijkstra(csgraph=mat, directed=True, indices=origin_sparse, return_predecessors=True, limit=100)
	...
	flat_index = [li for sublist in links_paths for li in sublist]
	...
	list_or_first = lambda x: list(x) if len(set(x)) > 1 else x.iloc[0]
	...
	n = len(tlinks[tlinks['oneway'].apply(lambda x: isinstance(x, list))])
	log.info(f'{n} links were not merge because the oneway field is not the same')
	tlinks = tlinks[~tlinks['oneway'].apply(lambda x: isinstance(x, list))]

	n = len(tlinks[tlinks['highway'].apply(lambda x: isinstance(x, list))])
	log.info(f'{n} links were not merge because the highway field is not the same')
	tlinks = tlinks[~tlinks['highway'].apply(lambda x: isinstance(x, list))]

	# TODO: on a des multiLinestring. il faudrait mieux merger.
	n = len(tlinks[tlinks['geometry'].apply(lambda x: x.geom_type != 'LineString')])
	log.info(f'{n} merged_links unmerged because the geometry became a multilinestring')
...
def split_duplicated_links(links: gpd.GeoDataFrame, nodes: gpd.GeoDataFrame) -> Tuple[gpd.GeoDataFrame, gpd.GeoDataFrame]:
	crs = nodes.crs
	...
	duplicated_links = duplicated_links.drop(columns=['geometry', 'middle_node']).rename(columns={'geometries': 'geometry'})
...
def drop_duplicated_links(links: gpd.GeoDataFrame, sort_column: str = 'maxspeed', ascending: bool = False) -> gpd.GeoDataFrame:
	...
	log.info(f'{before - len(links)} links dropped')
	return links
...
def clean_maxspeed(links: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
	...
	try:
		...
		log.error('fail to convert mph in maxspeed to float')
...
def clean_oneway(links: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
	...
	"""
	links.fillna({'oneway': 'no'}, inplace=True)
	links['oneway'] = (
		links['oneway']
		.replace('yes', True)
		.replace('no', False)
	...
	if not all(links['oneway'].isin([True, False])):
		log.warning('some oneway tags are not defined', links.unique())
	return links
...
def fill_na_col(links: gpd.GeoDataFrame, group: str = 'highway', col: str = 'maxspeed', agg_func: Callable[[list], Any] = np.mean) -> gpd.GeoDataFrame:
	"""
