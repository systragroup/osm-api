...
def main_strongly_connected_component(
	links: gpd.GeoDataFrame, nodes: gpd.GeoDataFrame = None, split_direction=True
) -> gpd.GeoDataFrame:
	"""
	...
	if nodes is not None:
		_nodes = nodes.loc[list(main_scc)]
	...
	for col1, col2 in [(a, b) for a in links['oneway'].unique() for b in links['highway'].unique()]:
		...
		for origins in batched(filtered_sources, 1000):
			...
			dist_matrix, predecessors = dijkstra(
				csgraph=mat, directed=True, indices=origin_sparse, return_predecessors=True, limit=cutoff
			)
...
def drop_duplicated_links(
	links: gpd.GeoDataFrame, sort_column: str = 'maxspeed', ascending: bool = False
) -> gpd.GeoDataFrame:
	"""
...
def clean_lanes(links: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
	...
	try:
		links.loc[semicolon_index, 'lanes'] = links.loc[semicolon_index, 'lanes'].apply(
			lambda x: np.mean([float(el) for el in x.split(';')])
		)
	except:
		pass
...
def fill_na_col(
	links: gpd.GeoDataFrame, group: str = 'highway', col: str = 'maxspeed', agg_func: Callable[[list], Any] = np.mean
) -> gpd.GeoDataFrame:
	"""
	...
	links.loc[~np.isfinite(links[col]), col] = links.loc[~np.isfinite(links[col]), group].apply(
		lambda x: val_dict.get(x)
	)
	return links
