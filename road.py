...
def process_list_in_col(col_values: Any,
                        new_type: Any,
                        function: Callable[[Any], Any] = np.nanmean,
                        ) -> Any:
    '''
...
def remove_list_in_col(col_values: Any,
                       method: str = 'first',
                       ):
    '''
...
@jit(nopython=True,
     locals={'predecessors': nb.int32[:, ::1], 'i': nb.int32, 'j': nb.int32}
     )
...
def get_links_and_nodes(geojson_file: str,
...
def simplify(links, cutoff=10):
    ...
    for col1, col2 in [(a, b) for a in links['oneway'].unique() for b in links['highway'].unique()]:
        ...
        for origins in batched(filtered_sources, 1000):
            ...
            dist_matrix, predecessors = dijkstra(csgraph=mat,
                                                 directed=True,
...
def split_duplicated_links(links: gpd.GeoDataFrame,
...
def fill_na_col(links: gpd.GeoDataFrame,
                group: str = 'highway',
                col: str = 'maxspeed',
                agg_func: Callable[[list], Any] = np.mean
                ) -> gpd.GeoDataFrame:
    '''
